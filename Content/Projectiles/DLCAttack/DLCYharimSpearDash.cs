...
namespace YharimEX.Content.Projectiles.DLCAttack
{
    ...
    {
        public override string Texture => "YharimEX/Content/Projectiles/DLCAttack/YharimAtlantis";
        ...
        {
            npc = YharimEXUtilities.NPCExists(reader.Read7BitEncodedInt());
        }
        ...
        public override void AI()
        {
            ...
            CalDLCEmodeAttacks mutantDLC = mutant.GetGlobalNPC<CalDLCEmodeAttacks>();
            if (mutant.active && mutant.type == ModContent.NPCType<YharimEXBoss>() && (mutantDLC.DLCAttackChoice == CalDLCEmodeAttacks.DLCAttack.BumbleDash || mutantDLC.DLCAttackChoice == CalDLCEmodeAttacks.DLCAttack.BumbleDash2))
            {
                int threshold = 40;
                ...
                if (mutantDLC.Timer >= threshold && (Projectile.ai[1] <= 0f || (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)) && --Projectile.localAI[0] < 0)
                {
                    ...
                    else if ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode))
                    {
        ...
        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
            ...
            if ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) && npc is NPC)
            {
                ...
                for (int i = 0; i < max; i++)
                {
                    ...
                    if (target.whoAmI == Main.myPlayer && target.ownedProjectileCounts[ModContent.ProjectileType<YharimEXHeal>()] < 10)
                    {
                        Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), target.Center, vel, ModContent.ProjectileType<YharimEXHeal>(), healPerOrb, 0f, Main.myPlayer, ai0, ai1);
        ...
        {
            Texture2D glow = ModContent.Request<Texture2D>("YharimEX/Assets/Projectiles/YharimEXEyeGlow", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
