...
namespace YharimEX.Content.Projectiles.MutantAttacks
{
    public class YharimEXGlowRingHollow : ModProjectile
    {
                ...
                case 3: //abom emode p2 dash telegraph
                    {
                        ...
                        NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[1], ModContent.NPCType<YharimEXBoss>());
                        if (npc != null)
                        {
                ...
                case 6: //destroyer coil tell
                    {
                        ...
                        NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[1], NPCID.TheDestroyer);
                        if (npc != null)
                        {
                            Projectile.Center = npc.Center;
                        }
                ...
                case 10: //nebula tower tp
                    {
                        ...
                        maxTime = 90;
                        alphaModifier = 10;
                        NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[1], NPCID.LunarTowerNebula);
                        if (npc != null)
                        {
                            if (Projectile.localAI[0] == maxTime)
                            {
                                ...
                                if (YharimEXUtilities.HostCheck)
                                {
                                    for (int i = -2; i <= 2; i++)
                                    {
                                        ...
                                        for (int j = -1; j <= 1; j += 2)
                                            Projectile.NewProjectile(npc.GetSource_FromThis(), spawnPoint, Vector2.Zero, ModContent.ProjectileType<YharimEXGlowLine>(), 0, 0f, Main.myPlayer, 15f, MathHelper.PiOver2 * j);
                                    }
                                }
                            }
                        }
                ...
                case 11: //lifelight crystalline tell
                    {
                        ...
                        maxTime = 30;
                        alphaModifier = 3;
                        NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[2], ModContent.NPCType<YharimEXBoss>());
                        if (npc != null)
                        {
                            Projectile.Center = npc.Center;
                        }
                ...
                case 13: //wof arena
                    {
                        ...
                        NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[1], NPCID.WallofFlesh);
                        if (npc != null)
                        {
                            Projectile.Center = npc.Center;
                        }
        ...
        public void DrawDestroyerTelegraph()
        {
            ...
            var diagonalNoise = YharimEXTextureRegistry.Techno1Noise;
            if (!blackTile.IsLoaded || !diagonalNoise.IsLoaded)
                return;
            var maxOpacity = 0.3f * Projectile.Opacity * (Main.mouseTextColor / 255f) * 0.9f;

            ManagedShader borderShader = ShaderManager.GetShader("YharimEX.YharimEXDestroyerCircleTelegraph");
