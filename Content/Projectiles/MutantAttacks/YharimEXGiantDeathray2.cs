    ...
    public class YharimEXGiantDeathray2 : YharimEXSpecialDeathray
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.localAI[0] == 0f)
            {
                if (!Main.dedServ)
                {
                    SoundEngine.PlaySound(new SoundStyle("YharimEX/Assets/Sounds/Attacks/YharimEXDeviBigDeathray") with { Volume = 1.5f }, Projectile.Center);
                    SoundEngine.PlaySound(new SoundStyle("YharimEX/Assets/Sounds/Attacks/YharimEXFinalSpark") with { Volume = 1.5f }, Projectile.Center);
                }
            }
            float num801 = 10f;
        ...
        {
            YharimEXPlayer modPlayer = target.GetModPlayer<YharimEXPlayer>();
            target.immune = false;
            target.immuneTime = 0;
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            ManagedShader shader = ShaderManager.GetShader("YharimEX.YharimEXDeathray");
            ...
            YharimEXUtilities.SetTexture1(YharimEXTextureRegistry.YharimEXStreak.Value);
            // Draw a big glow above the start of the laser, to help mask the intial fade in due to the immense width.

            Texture2D glowTexture = ModContent.Request<Texture2D>("YharimEX/Assets/Projectiles/YharimEXGlow").Value;
