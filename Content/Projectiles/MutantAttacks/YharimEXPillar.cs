...
namespace YharimEX.Content.Projectiles.MutantAttacks
{
    public class YharimEXPillar : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.alpha > 0)
            {
                ...
                if (Projectile.alpha <= 0)
                {
                    ...
                    if (target != -1)
                    {
                        SoundEngine.PlaySound(YharimEXSoundRegistry.YharimEXThrowShort with { Pitch = -0.5f }, Projectile.Center);
        ...
        public override void OnHitPlayer(Player target, Player.HurtInfo info)
        {
            ...
            target.AddBuff(FargoSouls.Find<ModBuff>("StunnedBuff").Type, 60);
        ...
        public override void OnKill(int timeLeft)
        {
            ...
            if (YharimEXUtilities.HostCheck)
            {
                ...
                float speed = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 5.5f : 5f;
                for (int j = 0; j < 4; j++)
                {
                    ...
                    for (int i = 0; i < max; i++)
                    {
                        int p = Projectile.NewProjectile(Terraria.Entity.InheritSource(Projectile), Projectile.Center, vel.RotatedBy(rotationInterval * i),
                            ModContent.ProjectileType<YharimEXFragment>(), Projectile.damage / 2, 0f, Main.myPlayer, Projectile.ai[0]);
