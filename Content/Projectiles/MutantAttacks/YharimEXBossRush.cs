...
namespace YharimEX.Content.Projectiles.MutantAttacks
{
    public class YharimEXBossRush : ModProjectile
    {
        ...
        {
            NPC npc = YharimEXUtilities.NPCExists(Projectile.ai[0], ModContent.NPCType<YharimEXBoss>());
            if (npc == null)
            {
                Projectile.Kill();
                return;
        ...
        private void ManualSpawn(NPC npc, int type)
        {
            ...
            {
                int n = YharimEXUtilities.NewNPCEasy(Terraria.Entity.InheritSource(Projectile), npc.Center, type);
                if (n != Main.maxNPCs)
                    YharimEXUtilities.PrintLocalization("Announcement.HasAwoken", Color.OrangeRed, Language.GetTextValue($"Mods.{Mod.Name}.NPCs.YharimEXBoss.DisplayName"));
            }
        }
    }
}