...
namespace YharimEX.Content.Projectiles.MutantAttacks
{
    public class YharimEXFishron : ModProjectile
    {
            ...
            else //preparing to dash
            {
                ...
                else //regular movement
                {
                    ...
                    if (Projectile.Distance(targetPos) < 50)
                        ...
                        Projectile.velocity = YharimEXUtilities.SmartAccel(Projectile.Center, targetPos, Projectile.velocity, 3f, 2f);
        ...
        private void SpawnRazorbladeRing(int max, float speed, float rotationModifier)
        {
            ...
            vel.Normalize();
            vel *= speed;
            int type = ModContent.ProjectileType<YharimEXTyphoon>();
            for (int i = 0; i < max; i++)
            {
