    ...
    public class PenetratorThrown : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            if (YharimEXCrossmodSystem.FargowiltasSouls.Loaded)
            {
                ...
                SetupFargoProjectile.noInteractionWithNPCImmunityFrames = true;
            ...
            if (--Projectile.localAI[0] < 0)
            {
                ...
                if (Projectile.owner == Main.myPlayer)
                {
                    ...
                    int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.Center, 16f * baseVel,
                        ModContent.ProjectileType<YharimEXPhantasmalSphere>(), Projectile.damage, Projectile.knockBack / 2, Projectile.owner, 1f);
        ...
        public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone)
        {
            if (Projectile.owner == Main.myPlayer)
            {
                int p = Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.position + new Vector2(Main.rand.Next(target.width), Main.rand.Next(target.height)), Vector2.Zero, ModContent.ProjectileType<YharimEXPhantasmalBlast>(), Projectile.damage, 0f, Projectile.owner);
                if (p != Main.maxProjectiles)
                    Main.projectile[p].DamageType = Projectile.DamageType;
            }
