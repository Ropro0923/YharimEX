...
namespace YharimEX.Content.Projectiles.MutantAttacks
{
	public class YharimEXHeal : ModProjectile, IPixelatedPrimitiveRenderer
    {
            ...
            else
            {
                ...
                if (Projectile.Distance(target.Center) < 5f)
                {
                    if (feedPlayer) //die and feed player
                    {
                        ...
                        {
                            Main.player[ai0].ClearBuff(FargoSouls.Find<ModBuff>("MutantFangBuff").Type);
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            //{
            //    Texture2D glow = texture2D13; //ModContent.Request<Texture2D>("FargowiltasSouls/Content/Projectiles/BossWeapons/PenetratorSpinGlow", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
            ...
            Main.EntitySpriteDraw(texture, Projectile.Center - Main.screenPosition, null, Color.Orange, Projectile.rotation, texture.Size() * 0.5f, Projectile.scale, SpriteEffects.None, 0);
            return false;
        }
        ...
        public void RenderPixelatedPrimitives(SpriteBatch spriteBatch)
        {
            ...
            YharimEXUtilities.SetTexture1(YharimEXTextureRegistry.FadedStreak.Value);
