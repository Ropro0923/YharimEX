...
using YharimEX.Common.Graphics.Particles;
...
{
    public class CalDLCEmodeAttacks : CalDLCEmodeAttacksBase
    {

        public override bool InstancePerEntity => true;
        public override GlobalNPC NewInstance(NPC target)
        {
            return YharimEXWorldFlags.EternityMode || YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.DeathMode || YharimEXWorldFlags.InfernumMode ? base.NewInstance(target) : null;
        }
        ...
        public override bool SafePreAI(NPC npc)
        {
            ...
            void ChooseNextAttack(params int[] args)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    int maxMemory = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 10 : 16;
                ...
                mutantBoss.endTimeVariance = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? Main.rand.NextFloat() : 0;
                ...
                if (npc.ai[1] == fishronDelay * maxFishronSets + 35)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        for (int j = -1; j <= 1; j += 2) //to both sides of player
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXOldDuke>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, offset.X, offset.Y);
                        }
                    }
                }
            }
            void CalamityMechRayFan()
            {
                ...
                int startTime = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 90 * 2 : 90;
                if (timer % 90 == 0 && timer > startTime)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        Projectile.NewProjectile(npc.GetSource_FromThis(), pos, vel, ModContent.ProjectileType<YharimEXPBG>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer);
                    }
                }
            }
            #endregion
            ...
            void CryogenP1()
            {
                ...
                if (--Timer < 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            int p = Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, velocity, ModContent.ProjectileType<IceBomb>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, 0f, 0f, 0f);
            ...
            void BumbleDrift()
            {
                ...
                if (Timer == WindupTime)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                        Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<DLCYharimSpearSpin>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, npc.whoAmI, DriftTime);
                }
            ...
            void BumbleDash()
            {
                ...
                if (Timer == 1)
                {
                    if (YharimEXUtilities.HostCheck)
                    {
                        Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<DLCYharimSpearDash>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, npc.whoAmI);
                    }
                }
                if (Timer < WindupTime)
                {
            ...
            void PrepareAresNuke()
            {
                ...
                */
                ...
                {
                    YharimEXBoss mutantBoss = (npc.ModNPC as YharimEXBoss);
                    Vector2 pos = YharimEXUtilities.ProjectileExists(mutantBoss.ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? npc.Center : Main.projectile[mutantBoss.ritualProj].Center;
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        if (!Main.projectile.Any(p => p != null && p.active && p.type == ritual))
                            Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ritual, YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 4f / 3f), 0f, Main.myPlayer, npc.whoAmI);
                        int nuke = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, distance, ModContent.ProjectileType<YharimEXAresNuke>(), (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 4f / 3f) : 0, 0f, Main.myPlayer, gravity);
                ...
                */
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Projectile.NewProjectile(npc.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXAresBomb>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 4f / 3f), 0f, Main.myPlayer);
                    }
                }
                ...
                if (++Timer > (Counter > 2 ? 80 : 30))
                {
                    if (++Counter > 3)
                    {
                        ChooseNextAttack(11, 13, 16, 19, 24, (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 26 : 29, 31, 35, 37, 39, 41, 42);
                        Counter = 0;
                        DLCAttackChoice = DLCAttack.None;
                    }
                    else
            ...
            void SlimeGodSlam()
            {
                ...
                {
                    SoundEngine.PlaySound(new SoundStyle("CalamityMod/Sounds/Custom/SlimeGodExit", (SoundType)0), npc.Center);
                ...
                if (Timer == 30 + Windup)
                {
                    SoundEngine.PlaySound(new SoundStyle("CalamityMod/Sounds/Custom/SlimeGodBigShot", 2, (SoundType)0), npc.Center);
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        for (int i = -8; i < 2; i++)
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, speedX * Vector2.UnitX + speedY * Vector2.UnitY, ModContent.ProjectileType<YharimEXSlimeGod>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 1f), 1f, Main.myPlayer, crimson);
                        }

                        float speed = 8;
                        Vector2 aureusVel = Vector2.Normalize(Vector2.UnitX * -Math.Sign(player.Center.X - npc.Center.X) + Vector2.UnitY) * speed;
                        Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, aureusVel, ModContent.ProjectileType<YharimEXAureusSpawn>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 1f), 1f, Main.myPlayer, player.whoAmI);
                    }
                    side = -side; //switch side
                    npc.netUpdate = true;
                }
                if (++Timer >= YharimEXSlimeGod.SlamTime + Windup)
                {
                    Timer = Windup;
                    if (++Counter >= 3)
                    {
            ...
            void Calamitas()
            {
                ...
                if (ModContent.TryFind(Calamity.Name, "BrimstoneMonster", out ModProjectile monster))
                {
                ...
                if (Timer == Startup)
                {
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        for (int i = 0; i < moons; i++)
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSource_FromAI(), targetPos, targetPos.DirectionTo(player.Center), brimstoneMonster, YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 4f / 3f), 0f, Main.myPlayer, 0f, 2f, 0f);
                        }
                    ...
                    if ((Timer - Startup) % CalamitasTime == CalamitasTime - 1)
                    {
                        if (YharimEXUtilities.HostCheck)
                        {
                            ...
                            for (int i = 0; i < calamiti; i++)
                            {
                                ...
                                Projectile.NewProjectile(npc.GetSource_FromAI(), spawnPos, aim, ModContent.ProjectileType<YharimEXSCal>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 4f / 3f), 0f, Main.myPlayer, TelegraphTime);
                            }
                        }
                    }
            ...
            void BumbleDrift2()
            {
                ...
                {
                    if (YharimEXUtilities.HostCheck)
                        Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, Vector2.Zero, ModContent.ProjectileType<DLCYharimSpearSpin>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, npc.whoAmI, WindupTime - StartupTimeOtherwiseItKindaTelefragsYouSometimes);
                }
            ...
            void BumbleDash2()
            {
                ...
                if (Timer == WindupTime)
                {
                    ...
                    foreach (Projectile projectile in Main.projectile.Where(p => p != null && p.active && p.type == ModContent.ProjectileType<DLCYharimSpearDash>()))
                    {
                        projectile.Kill();
                    }
                ...
                if (Timer > WindupTime && Timer % 6 == 0 && YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, -Vector2.Normalize(npc.velocity).RotatedByRandom(MathHelper.PiOver4), ModContent.ProjectileType<FrostMist>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, ai1: 55);
                }
            ...
            void Providence()
            {
                ...
                {
                    YharimEXBoss mutantBoss = (npc.ModNPC as YharimEXBoss);
                    Projectile arena = YharimEXUtilities.ProjectileExists(mutantBoss.ritualProj, ModContent.ProjectileType<YharimEXRitual>());
                    if (arena != null)
                    {
                ...
                else if (Timer - PrepareTime < DashTime)
                {
                    ...
                    if (Timer % 3 == 0)
                    {
                        if (YharimEXUtilities.HostCheck)
                        {
                            ...
                            Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, spearVel, ModContent.ProjectileType<HolySpear>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, 1f, 0f, 0f);
                        }
                    }
                }
                ...
                else if (Timer - PrepareTime - DashTime == LaserPrepareTime)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center.X, npc.Center.Y, velocity2.X, velocity2.Y, ModContent.ProjectileType<YharimEXHolyRay>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 3f / 2), 0f, Main.myPlayer, beamDirection * ((float)Math.PI * 2f) / rotation, npc.whoAmI, 0f);
                        Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center.X, npc.Center.Y, 0f - velocity2.X, 0f - velocity2.Y, ModContent.ProjectileType<YharimEXHolyRay>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage, 3f / 2), 0f, Main.myPlayer, (0f - beamDirection) * ((float)Math.PI * 2f) / rotation, npc.whoAmI, 0f);
                    }
                    npc.netUpdate = true;
                }
                else
                {
                    ...
                    {
                        Reset();
                        ChooseNextAttack(11, 16, 19, 20, (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 44 : 26, 31, 33, 35, 42, 44, 45);
                        return;
                    }
                }
                Timer++;
            ...
            void YharonBH()
            {
                ...
                const int EndTime = YharimEXYharonVortex.ThrowTime;
                if (Timer < WindupTime)
                {
                    npc.velocity *= 0.9f;
                }
                ...
                if (Timer == WindupTime)
                {
                    ...
                    int type = ModContent.ProjectileType<YharimEXYharonVortex>();
                    if (Main.netMode != NetmodeID.MultiplayerClient)
                    {
                        Projectile.NewProjectile(npc.GetSource_FromAI(), npc.Center, Vector2.Zero, type, YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, bhTime, npc.whoAmI, 0f);
                    }
                ...
                if (Timer > WindupTime && Timer <= WindupTime + bhTime)
                {
                    ...
                    int totalProjectiles = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 36 : 30;
                    if (Timer % flareDustSpawnDivisor == 0)
                    {
                        DoFlareDustBulletHell(0, flareDustSpawnDivisor, YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), totalProjectiles, 0f, 0f, phase2: true);
                    }
                ...
                if (++npc.ai[1] > 60)
                {
                    ...
                    else
                    {
                        ...
                        if (YharimEXUtilities.HostCheck) //spawn worm
                        {
                            ...
                            if ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode))
                                ai1 += 0.4f;
                            int current = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vel, ModContent.ProjectileType<YharimEXDoGHead>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, npc.target, ai1);
                            ...
                            for (int i = 0; i < max; i++)
                                current = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vel, ModContent.ProjectileType<YharimEXDoGBody>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity);
                            int previous = current;
                            current = Projectile.NewProjectile(npc.GetSource_FromThis(), npc.Center, vel, ModContent.ProjectileType<YharimEXDoGTail>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity);
            ...
            void Polterghast()
            {
                ...
                if (Timer % PolterTime == (PolterTime / 2))
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        for (int i = 0; i < Polters; i++)
                        {
                            ...
                            Vector2 spawnPos = player.Center + (spawnDir * YharimEXPolter.StartDistance);
                            Vector2 targetPos = player.Center;
                            Projectile.NewProjectile(npc.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXPolter>(), YharimEXUtilities.ScaledProjectileDamage(npc.defDamage), 0f, Main.myPlayer, targetPos.X, targetPos.Y, Counter);
                        }
                    }
                    Counter = -Counter;
                    npc.ai[3] = Main.rand.NextFloat(MathHelper.TwoPi);
                ...
                {
                    ChooseNextAttack(11, 13, 16, 21, (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 26 : 24, 29, 31, 35, 37, 39, 41, 45);
                    Reset();
                    return;
                }
                Timer++;
