...
using YharimEX.Content.NPCs.Godseeker;
using YharimEX.Content.NPCs.YharimIllusion;

namespace YharimEX.Content.NPCs.YharimBoss
{
    [AutoloadBossHead]
    public class YharimEXBoss : ModNPC
    {
        ...
        public override void SetDefaults()
        {
            ...
            NPC.lifeMax = Main.expertMode ? 5500000 : 2400000;
        ...
        public override bool CanHitPlayer(Player target, ref int CooldownSlot)
        {
            ...
            if (!(YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode))
                return false;
            return NPC.Distance(YharimEXUtilities.ClosestPointInHitbox(target, NPC.Center)) < Player.defaultHeight && AttackChoice > -1;
        }
        ...
        public override void AI()
        {
            ...
            //drop summon
            if ((YharimEXWorldFlags.EternityMode || YharimEXWorldFlags.DeathMode) && !YharimEXWorldFlags.DownedYharimEX && YharimEXUtilities.HostCheck && NPC.HasPlayerTarget && !droppedSummon)
            {
        ...
        void ManageAurasAndPreSpawn()
        {
            ...
            if (NPC.localAI[3] == 0)
            {
                ...
                if (NPC.Distance(Main.player[NPC.target].Center) < 1500)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
        ...
        {
            if (YharimEXUtilities.HostCheck) //checks for needed projs
            {
                if ((YharimEXWorldFlags.EternityMode || YharimEXWorldFlags.DeathMode) && AttackChoice != -7 && (AttackChoice < 0 || AttackChoice > 10) && YharimEXUtilities.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null)
                    ritualProj = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXRitual>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 0f, NPC.whoAmI);

                if (YharimEXUtilities.ProjectileExists(ringProj, ModContent.ProjectileType<YharimEXRitual5>()) == null)
                ...
                if (YharimEXUtilities.ProjectileExists(spriteProj, ModContent.ProjectileType<YharimEXBossProjectile>()) == null)
                {
                    /*if (Main.netMode == NetmodeID.Server)
            ...
            if (forceDespawn || (!p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f) && NPC.localAI[3] > 0)
            {
                ...
                if (forceDespawn || !p.active || p.dead || Vector2.Distance(NPC.Center, p.Center) > 3000f)
                {
                    ...
                    if (NPC.timeLeft == 1)
                    {
                        ...
                        if (NPC.position.Y < 0)
                            NPC.position.Y = 0;
                        if (YharimEXUtilities.HostCheck && !NPC.AnyNPCs(ModContent.NPCType<TheGodseeker>()) && YharimEXWorldFlags.DownedYharimEX)
                        {
                            YharimEXUtilities.ClearHostileProjectiles(2, NPC.whoAmI);
        ...
        bool Phase2Check()
        {
            if (Main.expertMode && NPC.life < NPC.lifeMax * (2f / 3))
            {
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    NPC.ai[3] = 0;
                    NPC.netUpdate = true;
                    YharimEXUtilities.ClearHostileProjectiles(1, NPC.whoAmI);
        ...
        private void EdgyBossText(string text)
        {
            if (Main.zenithWorld) //edgy boss text
            {
                ...
                YharimEXUtilities.PrintText(text, color);
        ...
        void SpearTossDirectP1AndChecks()
        {
            ...
            if (NPC.ai[1] > 150) //120)
            {
                ...
                else if (YharimEXUtilities.HostCheck)
                {
                    Vector2 vel = NPC.localAI[0].ToRotationVector2() * 25f;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXSpearThrown>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target);
                    if (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                    }
                }
                NPC.localAI[0] = 0;
            }
            else if (NPC.ai[1] == 61 && NPC.ai[2] < NPC.ai[3] && YharimEXUtilities.HostCheck)
            {
                if ((YharimEXWorldFlags.EternityMode || YharimEXWorldFlags.DeathMode) && YharimEXWorldFlags.SkipYharimEXP1 >= 10 && !(YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode))
                {
                    ...
                    if (YharimEXWorldFlags.SkipYharimEXP1 == 10)
                        YharimEXUtilities.PrintLocalization($"Mods.{Mod.Name}.NPCs.SkipPhase1", Color.OrangeRed);
                ...
                if ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) && NPC.ai[2] == 0) //first time only
                {
                    ...
                    if (YharimEXUtilities.HostCheck) //spawn worm
                    {
                        int appearance = 0;
                        for (int j = 0; j < 8; j++)
                        {
                            ...
                            int current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerHead>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, ai1, appearance);
                            ...
                            for (int i = 0; i < max; i++)
                                current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerBody>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity, 0f, appearance);
                            int previous = current;
                            current = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXDestroyerTail>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[current].identity, 0f, appearance);
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 3);
        ...
        void OkuuSpheresP1()
        {
            ...
            if (--NPC.ai[1] < 0)
            {
                ...
                else
                {
                    ...
                    SpawnSphereRing(max, speed, (int)(0.8 * YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage)), 1f * sign);
                    SpawnSphereRing(max, speed, (int)(0.8 * YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage)), -0.5f * sign);
                }
            ...
            if (--NPC.ai[1] < 0)
            {
                ...
                else if (NPC.ai[2] <= maxEyeThreshold)
                {
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, type, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer, NPC.target);
        ...
        void PrepareSpearDashDirectP1()
        {
            ...
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 240); // 250);
        ...
        void SpearDashDirectP1()
        {
            ...
            if (++NPC.ai[1] > NPC.ai[3])
            {
                ...
                else
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);

                        if (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        }
                    }
        ...
        void VoidRaysP1()
        {
            ...
            {
                if (YharimEXUtilities.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(2, 0).RotatedBy(NPC.ai[2]), ModContent.ProjectileType<YharimEXMark1>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        ...
                        if (YharimEXUtilities.HostCheck)
                        {
                            ...
                            for (int i = 0; i < max; i++)
                            {
                                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, new Vector2(0f, -7f).RotatedBy(NPC.ai[2] + MathHelper.TwoPi / max * i),
                                    ModContent.ProjectileType<YharimEXEye>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                            }
                        }
            ...
            if (NPC.ai[2] == 0) //move onscreen so player can see
            {
                ...
                if (++NPC.localAI[0] > 30 || (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) && NPC.Distance(targetPos) < 64)
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        ...
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + pos, Vector2.Zero, ModContent.ProjectileType<YharimEXSword>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, NPC.whoAmI, spacing);
                        }
                    ...
                    for (int i = 0; i < max; i++)
                    {
                        ...
                        if (YharimEXUtilities.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos + Main.rand.NextVector2Circular(NPC.width / 2, NPC.height / 2), Vector2.Zero, ModContent.ProjectileType<YharimEXSunBlast>(),
                                YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, MathHelper.WrapAngle(angle.ToRotation()), ai1);
                        }
                    }
                }
            }
        ...
        void Phase2Transition()
        {
            ...
            if (NPC.ai[2] == 0)
            {
                if (NPC.ai[1] < 60 && !Main.dedServ && Main.LocalPlayer.active)
                    YharimEXUtilities.ScreenshakeRumble(6);
            }
            else
            {
                NPC.velocity = Vector2.Zero;
            ...
            if (NPC.ai[1] == 0)
            {
                YharimEXUtilities.ClearAllProjectiles(2, NPC.whoAmI);
        ...
        void PrepareSpearDashPredictiveP2()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 180); // + 60);
            ...
            if (NPC.ai[1] == 0) //telegraph
            {
                ...
                if (NPC.ai[2] < NPC.localAI[1])
                {
                    ...
                    if (NPC.ai[2] == NPC.localAI[1] - 1)
                    {
                        ...
                        if (YharimEXUtilities.HostCheck)
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 4);
                    }
                }
            }
            ...
            if (++NPC.ai[1] > endTime)
            {
                ...
                else
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, spearAi);
                    }
        ...
        void PillarDunk()
        {
            ...
            {
                SoundEngine.PlaySound(SoundID.Roar, NPC.Center);
                if (YharimEXUtilities.HostCheck) //spawn cultists
                {
                    void Clone(float ai1, float ai2, float ai3) => YharimEXUtilities.NewNPCEasy(NPC.GetSource_FromAI(), NPC.Center, ModContent.NPCType<YharimEXIllusion>(), NPC.whoAmI, NPC.whoAmI, ai1, ai2, ai3);
            ...
            else if (NPC.ai[1] == pillarAttackDelay)
            {
                if (YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -5,
                        ModContent.ProjectileType<YharimEXPillar>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0, Main.myPlayer, 3, NPC.whoAmI);
                }
            }
            else if ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) && NPC.ai[1] == pillarAttackDelay * 5)
            {
                ...
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -5,
                        ModContent.ProjectileType<YharimEXPillar>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0, Main.myPlayer, 1, NPC.whoAmI);
                }
            }
        }
        ...
        void EOCStarSickles()
        {
            ...
            if (NPC.ai[1] == 0)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.UnitY, ModContent.ProjectileType<YharimEXEyeOfCthulhu>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, ai1);
        ...
        void PrepareSpearDashDirectP2()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearSpin>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 180);// + (YharimEXWorldFlags.MasochistMode ? 10 : 20));
                    TelegraphSound = SoundEngine.PlaySound(YharimEXSoundRegistry.YharimEXUnpredictive with { Volume = 2f }, NPC.Center);
                }
        ...
        void SpearDashDirectP2()
        {
            ...
            if (++NPC.ai[1] > ((YharimEXWorldFlags.EternityMode || YharimEXWorldFlags.DeathMode) ? 5 : 20))
            {
                ...
                else
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(NPC.velocity), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearDash>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
                    }
                }
        ...
        void SpearTossPredictiveP2()
        {
            ...
            if (++NPC.ai[1] > 60)
            {
                ...
                if ((shouldAttack || (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)) && YharimEXUtilities.HostCheck)
                {
                    Vector2 vel = NPC.SafeDirectionTo(player.Center + player.velocity * 30f) * 30f;
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, -Vector2.Normalize(vel), ModContent.ProjectileType<YharimEXDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.8f), 0f, Main.myPlayer);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel, ModContent.ProjectileType<YharimEXSpearThrown>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.target, 1f);
                }
            }
            else if (NPC.ai[1] == 1 && (NPC.ai[2] < NPC.localAI[1] || (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)) && YharimEXUtilities.HostCheck)
            {
                ...
                Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 2);
            }
        }
        ...
        void MechRayFan()
        {
            ...
            if (NPC.ai[3] == 0 && YharimEXUtilities.HostCheck)
            {
                int max = 7;
                for (int i = 0; i <= max; i++)
                {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    void SpawnRay(Vector2 pos, float angleInDegrees, float turnRotation)
                    {
                        int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), pos, MathHelper.ToRadians(angleInDegrees).ToRotationVector2(),
                            ModContent.ProjectileType<YharimEXDeathray3>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0, Main.myPlayer, turnRotation, NPC.whoAmI);
            ...
            void SpawnPrime(float varianceInDegrees, float rotationInDegrees)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel, ModContent.ProjectileType<YharimEXGuardian>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer);
                }
            }
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    for (int j = -1; j <= 1; j += 2) //to both sides of player
                    {
                        ...
                        for (int i = -max; i <= max; i++) //fan of fishron
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, projType, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, offset.X, offset.Y);
                        }
                    }
                }
                for (int i = 0; i < 30; i++)
        ...
        void PrepareNuke()
        {
            ...
            if (++NPC.ai[1] > 60)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, distance, ModContent.ProjectileType<YharimEXNuke>(), (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f) : 0, 0f, Main.myPlayer, gravity);
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXFishronRitual>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer, NPC.whoAmI);
                }
                AttackChoice++;
                NPC.ai[1] = 0;
            ...
            if (NPC.ai[1] > ((YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 120 : 180))
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXBomb>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 4f / 3f), 0f, Main.myPlayer);
                    }
                }
            }
        ...
        void SlimeRain()
        {
            if (NPC.ai[3] == 0)
            {
                ...
                SoundEngine.PlaySound(SoundID.Roar, NPC.Center);
                if (YharimEXUtilities.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSlimeRain>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI);
            }
            ...
            if (NPC.ai[1] > 120 && NPC.ai[1] % 5 == 0) //rain down slime balls
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    void Slime(Vector2 pos, float off, Vector2 vel)
                    {
                        ...
                        float ai0 = YharimEXUtilities.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? 0f : NPC.Distance(Main.projectile[ritualProj].Center);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel * flip * 2 /* x2 to compensate for removed extraUpdates */, ModContent.ProjectileType<YharimEXSlimeBall>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0);
                    }
            ...
            if (NPC.ai[1] > masoMovingRainAttackTime && NPC.ai[1] % 3 == 0) //rain down slime balls
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    void Slime(Vector2 pos, float off, Vector2 vel)
                    {
                        ...
                        Vector2 spawnPos = pos + off * Vector2.UnitY * flip;
                        float ai0 = YharimEXUtilities.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? 0f : NPC.Distance(Main.projectile[ritualProj].Center);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, vel * flip, ModContent.ProjectileType<YharimEXSlimeSpike>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0, ai2: frame);
                    }
        ...
        void OkuuSpheresP2()
        {
            ...
            if (++NPC.ai[1] > 10 && NPC.ai[3] > 60 && NPC.ai[3] < endTime - 60)
            {
                ...
                SpawnSphereRing(max, speed, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), -1f, rotation);
                SpawnSphereRing(max, speed, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 1f, rotation);
        ...
        void SpearTossDirectP2()
        {
            ...
            {
                if (NPC.ai[2] > 0 && (NPC.ai[2] < NPC.localAI[1] || (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode)) && YharimEXUtilities.HostCheck)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.Zero, ModContent.ProjectileType<YharimEXSpearAim>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, NPC.whoAmI, 1);
            }
            else if (NPC.ai[1] == 1)
            {
                if (YharimEXUtilities.HostCheck)
        ...
        void TwinRangsAndCrystals()
        {
            ...
            if (NPC.ai[3] == 0)
            {
                ...
                if (!(YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) && YharimEXUtilities.HostCheck)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + Vector2.UnitX.RotatedBy(Math.PI / 2 * i) * 525, Vector2.Zero, ModContent.ProjectileType<YharimEXGlowRingHollow>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 1f);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center + Vector2.UnitX.RotatedBy(Math.PI / 2 * i + Math.PI / 4) * 350, Vector2.Zero, ModContent.ProjectileType<YharimEXGlowRingHollow>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, 2f);
                    }
                }
            }
            ...
            if (NPC.ai[3] % ringDelay == 0 && NPC.ai[3] < ringDelay * ringMax)
            {
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    int p = Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, 300f / baseDelay * Vector2.UnitX.RotatedBy(rotationOffset), ModContent.ProjectileType<YharimEXMark2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, baseDelay, baseDelay + flyDelay);
                    if (p != Main.maxProjectiles)
                    {
                        ...
                        for (int i = 0; i < max; i++)
                        {
                            ...
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), spawnPos, Vector2.Zero, ModContent.ProjectileType<YharimEXCrystalLeaf>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, Main.projectile[p].identity, myRot);
                        }
                    }
                }
            }
            ...
            if (NPC.ai[3] > 45 && --NPC.ai[1] < 0)
            {
                ...
                if (YharimEXUtilities.HostCheck && NPC.ai[3] < 330)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(Math.PI / 2 * i + rotationOffset) * retiSpeed, ModContent.ProjectileType<YharimEXRetirang>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, retiAcc, 300);
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(Math.PI / 2 * i + Math.PI / 4 + rotationOffset) * spazSpeed, ModContent.ProjectileType<YharimEXSpazmarang>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, spazAcc, 180);
                    }
                }
            }
            if (++NPC.ai[3] > 450)
            ...
            void Sword(Vector2 pos, float ai0, float ai1, Vector2 vel)
            {
                if (YharimEXUtilities.HostCheck)
                {
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), pos - vel * 60f, vel,
                        ProjectileID.FairyQueenLance, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer, ai0, ai1);
                }
            }
        ...
        void SANSGOLEM()
        {
            ...
            if (NPC.ai[1] > 0 && NPC.ai[1] % attackDelay == 0)
            {
                ...
                Vector2 centerPoint = YharimEXUtilities.ProjectileExists(ritualProj, ModContent.ProjectileType<YharimEXRitual>()) == null ? player.Center : Main.projectile[ritualProj].Center;
                    ...
                    for (int j = -1; j <= 1; j += 2) //flappy bird tubes
                    {
                        ...
                        if (YharimEXUtilities.HostCheck)
                        {
                            Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, vel,
                                ...
                                YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer,
        ...
        void VoidRaysP3()
        {
            if (--NPC.ai[1] < 0)
            {
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, speed * Vector2.UnitX.RotatedBy(NPC.ai[2]), ModContent.ProjectileType<YharimEXMark1>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0f, Main.myPlayer);
                }
                NPC.ai[1] = 1;
                NPC.ai[2] += NPC.ai[3];
        ...
        void OkuuSpheresP3()
        {
            ...
            if (++NPC.ai[1] > 10 && NPC.ai[3] > 60 && NPC.ai[3] < endTime - 120)
            {
                ...
                float speed = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? 11f : 10f;
                SpawnSphereRing(max, speed, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), -0.75f, rotation);
                SpawnSphereRing(max, speed, YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage), 0.75f, rotation);
            }
            ...
            */
            ...
            if (++NPC.ai[1] > ringTime)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                {
                    ...
                    int damage = YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage);
                ...
                if (NPC.localAI[1] == 0) //maso do ordinary spark
                {
                    ...
                    if (YharimEXUtilities.HostCheck)
                    {
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center, Vector2.UnitX.RotatedBy(NPC.ai[3]),
                            ModContent.ProjectileType<YharimEXGiantDeathray2>(), YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.5f), 0f, Main.myPlayer, 0, NPC.whoAmI);
                    }
            ...
            else
            {
                ...
                if (NPC.ai[1] % 3 == 0 && YharimEXUtilities.HostCheck)
                {
        ...
        void DyingDramaticPause()
        {
            ...
            else if (++NPC.ai[1] > 120)
            {
                ...
                NPC.netUpdate = true;
                if (YharimEXUtilities.HostCheck) //shoot death anim mega ray
                {
                    int damage = (YharimEXWorldFlags.MasochistModeReal || YharimEXWorldFlags.InfernumMode) ? YharimEXUtilities.ScaledProjectileDamage(NPC.defDamage, 0.5f) : 0;
        ...
        void DyingAnimationAndHandling()
        {
            ...
            if (++NPC.alpha > 255)
            {
                ...
                NPC.checkDead();
                if (YharimEXUtilities.HostCheck && ModContent.TryFind("YharimEX", "TheGodseeker", out ModNPC modNPC) && !NPC.AnyNPCs(modNPC.Type))
                {
        ...
        public override bool CheckDead()
        {
            ...
            NPC.life = 1;
            NPC.active = true;
            if (YharimEXUtilities.HostCheck && AttackChoice > -1)
            {
                ...
                NPC.dontTakeDamage = true;
                NPC.netUpdate = true;
                YharimEXUtilities.ClearAllProjectiles(2, NPC.whoAmI, AttackChoice < 0);
        ...
        public static void ArenaAura(Vector2 center, float distance, bool reverse = false, int dustid = -1, Color color = default, params int[] buffs)
        {
            ...
            if (p.active && !p.dead && !p.ghost && (reverse ? range > distance && range < Math.Max(3000f, distance * 2) : range < distance))
            {
                ...
                {
                    YharimEXUtilities.AddDebuffFixedDuration(p, buff, 2);
                }
            }
        }
    }
