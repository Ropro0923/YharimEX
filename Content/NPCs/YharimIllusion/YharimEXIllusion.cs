...
namespace YharimEX.Content.NPCs.YharimIllusion
{
    public class YharimEXIllusion : ModNPC
    {
        ...
        {
            NPC mutant = YharimEXUtilities.NPCExists(NPC.ai[0], ModContent.NPCType<YharimEXBoss>());
            ...
            if (--NPC.ai[3] == 0)
            {
                ...
                if (YharimEXUtilities.HostCheck)
                    Projectile.NewProjectile(mutant.GetSource_FromThis(), NPC.Center, Vector2.UnitY * -5, ModContent.ProjectileType<YharimEXPillar>(), YharimEXUtilities.ScaledProjectileDamage(mutant.damage, 4f / 3), 0, Main.myPlayer, ai0, NPC.whoAmI);
            }
